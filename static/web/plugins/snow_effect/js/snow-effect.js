...
goAdorSnow('', 1, 'wolken1.png', true, true, false, 0.5, 0.5, 0.4);
$("#setMist").click(function (event) {
	if (this.checked) {
		adEnableSnowBackground(true);
		$("#code_mist").text("true");
	} else {
		adEnableSnowBackground(false);
		$("#code_mist").text("false");
	}
});
$("#setBgrClouds").change(function () {
	var clouds = $(this).children("option:selected").val();
	adSetBackgroundImage(clouds);
	if (!clouds) {
		$("#code_bgr_clouds").text("false");
	} else {
		$("#code_bgr_clouds").text("true");
	}
	$("#code_clouds").text(clouds);
});
$("#setFlash").click(function (event) {
	if (this.checked) {
		adEnableFlashes(true);
		$("#code_flash").text("true");
	} else {
		adEnableFlashes(false);
		$("#code_flash").text("false");
	}
});
$("#setZindex").click(function (event) {
	if (this.checked) {
		$("#code_zindex").text("-1000");
	} else {
		$("#code_zindex").text("1000");
	}
});
$("#setDiv").keyup(function () {
	var div = $(this).val();
	$("#code_div").text(div);
});
$(function () {
	$("#slider_amount").slider({
		...
		slide: function (event, ui) {
			adSetSnowAmount(ui.value);
			$("#code_amount").text(ui.value);
		}
	});
	$("#slider_turbulence").slider({
		value: 0.4,
		min: 0.0,
		max: 1.0,
		step: 0.1,
		slide: function (event, ui) {
			adSetTurbulence(ui.value);
			$("#code_turbulence").text(ui.value);
		}
	});
	$("#slider_rateOfFall").slider({
		value: 0.5,
		min: 0.0,
		max: 1.0,
		step: 0.1,
		slide: function (event, ui) {
			adSetFallSpeed(ui.value);
			$("#code_rate_of_fall").text(ui.value);
		}
	});
});
